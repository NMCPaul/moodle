/**
 * Customises the dock for the nmc20 theme and does some other cool stuff
 */
function customise_dock_for_theme() {
    ...
    M.core_dock.on('dock:drawcompleted', nmc20.dock.enable_side_switching, nmc20.dock);
    // When the dock is first drawn check to see if it should be moved
    M.core_dock.on('dock:drawstarted', nmc20.dock.check_initial_position, nmc20.dock);
    // Corrects the panel x position for the theme
    M.core_dock.on('dock:itemadded', function(item) {
        item.on('dockeditem:showstart', nmc20.dock.correct_panel_x_position, nmc20.dock, item);
        item.on('dockeditem:resizecomplete', nmc20.dock.correct_panel_x_position, nmc20.dock, item);
    });

    // Override the default fix_title_orientation method with our nmc20 method
    // this will use SVG and rotate the text if possible.
    M.core_dock.genericblock.prototype.fix_title_orientation = nmc20.dock.fix_title_orientation;
    M.core_dock.genericblock.prototype.resize_block_space = nmc20.dock.resize_block_space;
    return true;
}
    ...
    return {
        ...
        dock : {
            ...
            fix_title_orientation : function (node) {
                if (M.core_dock.cfg.orientation == 'vertical') {
                    return nmc20.transform.make_vertical_text(node);
                }
                return node;
            },
        ...
        transform : {
            make_vertical_text : function(node) {
                ...
                var txt = document.createElementNS(nmc20.namespaces.svg, 'text');
                txt.setAttribute('x', '0');
                txt.setAttribute('y', '0');
                txt.setAttribute('transform','rotate(90, 5, 5)');
                txt.appendChild(document.createTextNode(node.firstChild.nodeValue));

                var svg = document.createElementNS(nmc20.namespaces.svg, 'svg');
