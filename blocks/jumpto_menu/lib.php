<?php
    function block_jumpto_menu_html()
    {
     ...
     if ($PAGE->cm)
         return block_jumpto_menu_navmenu($COURSE, true, $PAGE->cm, $target, false, "");
     else
         return block_jumpto_menu_navmenu($COURSE, true, NULL, $target, false, "");
    }
     ...
     */
    function block_jumpto_menu_navmenu($course, $nav_buttons=true, $cm=NULL, $targetwindow="window", $scriptembed=true, $instance) {
        ...
        foreach ($modinfo->cms as $mod) {
            ...
            if (empty($THEME->navmenuiconshide)) {
                ...
                $menustyle[$url] = 'style="background-image: url('.$OUTPUT->pix_url('icon', $mod->modname).');"';
            }
            $previousmod = $mod;
        }
...
*/
        ...
        $popup=block_jumpto_menu_popup_form($CFG->wwwroot .'/mod/', $menu, 'navmenupopup', $selected, $strjumpto,
                       '', '', true, $targetwindow, '', $menustyle);

        $divtext='<div ';
        //if ($scriptembed)
        //    $divtext.='style="float:right;" ';
        $divtext.='class="jumpto_menu" style="text-align:center;margin-top:2px;">'.'<ul>'.$logslink . $backmod .
            '<li>'.$popup.'</li>'.
            $nextmod . '</ul>'.'</div>';
        ...
        $final.=block_jumpto_menu_basic_script($targetwindow, $scriptembed, $instance);

        return $final;
    }

    function block_jumpto_menu_basic_script($targetwindow, $scriptembed, $instance)
    {
        global $CFG, $PAGE;
        $final="<script type=\"text/javascript\">\n".
           " //<!--\n".
     ...
     */
    function block_jumpto_menu_popup_form($common, $options, $formid, $selected='', $nothing='choose', $help='', $helptext='', $return=false,
