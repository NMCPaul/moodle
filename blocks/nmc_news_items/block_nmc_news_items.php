...
class block_nmc_news_items extends block_base {
    function init() {
        $this->title = get_string('pluginname', 'block_nmc_news_items');
    }
    ...
    function get_content() {
        ...
        /// Actually create the listing now
            ...
            $strmore = get_string('more', 'block_nmc_news_items');
            $strfull = get_string('full', 'block_nmc_news_items');
        ...
        /// Accessibility: markup as a list.
            ...
            foreach ($discussions as $discussion) {
                ...
                $date = userdate($discussion->modified, $strftimerecent);

                $word_count = 20;

                $message = '<p>';
                $allowedCharacters = '1234567890?!.,&"@$%()*';
                if(str_word_count(strip_tags($discussion->message), 0) > $word_count) {
                    $message_array = str_word_count(strip_tags($discussion->message), 1, $allowedCharacters);
                ...
                $text .= '<li class="post">'.
                         ...
                         '<div class="title" title="Posted: '.$date.'">'.ucwords($discussion->subject).'</div>'.
                         //'<div class="name">'.fullname($discussion).'</div></div>'.
                         '<div class="info">'.$message.' '.
                         '<div class="full">(<a href="'.$CFG->wwwroot.'/mod/forum/discuss.php?d='.$discussion->discussion.'">'.
                         $strfull.'</a>)</div></div>'.
                         "</li>\n";
            }
            $text .= "</ul>\n";
            ...
            $this->content->footer = '<a href="'.$CFG->wwwroot.'/mod/forum/view.php?f='.$forum->id.'">'.
                                      get_string('all_announcements', 'block_nmc_news_items').'</a>';
